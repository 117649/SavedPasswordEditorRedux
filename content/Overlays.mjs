...
export class Overlays {
  ...
  constructor(overlayProvider, window) {
    ...
    if (!this.sandboxes.getSandbox) this.sandboxes.getSandbox = function (obj) {
      let global = Cu.getGlobalForObject(obj);
      if (this.has(global)) return this.get(global);
  ...
  _insertInlineEventHandler = (node, textContent) => Cu.evalInSandbox(`(function(event){${textContent}})`, this.sandboxes.getSandbox(node));
